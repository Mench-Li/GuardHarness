...
### 7. Sandbox 自动隔离工作区（v2.6 新增）

执行计划时，Agent-Guard **自动创建 git worktree 沙盒**，每个任务在独立分支上运行，互不干扰：

- **自动创建**：`execute` 命令默认自动调用 `sandbox create`，在 `.worktrees/` 下创建 `task/{task-id}` 分支
- **自动销毁**：`finish` 命令完成后自动提取 patch 并销毁沙盒
- **G4 Surgical Check 感知沙盒**：diff 检查在沙盒工作目录内执行，确保只修改计划内文件
- **Snapshot 记录沙盒路径**：中断恢复时自动加载沙盒上下文
- **手动跳过**：`python .harness/agent-guard/cli.py execute TASK-001 --no-sandbox`

```bash
# 手动管理沙盒
python .harness/agent-guard/cli.py sandbox create TASK-001
python .harness/agent-guard/cli.py sandbox destroy TASK-001 --patch
```
...
# 沙盒管理
python .harness/agent-guard/cli.py sandbox create  <task-id>
python .harness/agent-guard/cli.py sandbox destroy <task-id> [--patch]
