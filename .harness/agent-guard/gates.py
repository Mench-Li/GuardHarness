...
def g4_surgical_check(task_id: str, **kwargs: Any) -> dict[str, Any]:
    """G4: Verify diff only touches task-related files.

    Phase 1 simplified: checks for modified files against a scope list.
    Full version would compare git diff against plan file_changes.
    """
    try:
        result = subprocess.run(
            ["git", "diff", "--name-only"],
        ...
        return {
            ...
            "details": {"error": str(e)},
            "blocking": GATE_BLOCKING.get("g4_surgical_check", False),
        }

    modified = [line.strip() for line in result.stdout.splitlines() if line.strip()]

    # If no modifications, that's fine (maybe nothing to verify yet)
    if not modified:
        return {
            "passed": True,
    ...
    # Phase 1: just record modified files, don't block
    # In full version, compare against planned file list
    return {
        "passed": True,
        "message": f"Found {len(modified)} modified files (surgical check in advisory mode)",
        "details": {"modified_files": modified},
        "blocking": GATE_BLOCKING.get("g4_surgical_check", False),
    }


def g5_verification_proof(task_id: str, verification_command: str | None = None, **kwargs: Any) -> dict[str, Any]:
