...
| G4 Surgical Check | Executing → Patch Ready | 检测 staged + unstaged + untracked；只在 plan 的 `file_changes` 节匹配允许文件；过滤运行时产物；沙盒环境下在 worktree 内检查 | **硬阻断** |
...
python .harness/agent-guard/cli.py execute  [<task-id>] [--no-sandbox]  # 省略则自动认领
...
- **TDD 硬宪法规则（v2.6）**：每个 Task 必须严格遵循测试优先顺序——写失败测试 → 运行确认失败 → 写最小实现 → 运行确认通过。禁止先实现后补测试，违反视为 plan 不合规（G1/G2 可据此否决）
- 最大步骤时长：5 分钟
- 必须包含验证命令
...
- **G4 Surgical Check**（硬阻断）：验证 git diff 只修改计划内文件
  - 检测 staged + unstaged + untracked 三类变更
  - 自动过滤 Agent-Guard 运行时产物（state/snapshots/leases）
  - 只在 plan 的 `file_changes` 章节匹配允许文件（严格匹配，不在其他章节误匹配）
  - **v2.7 升级**：从建议升级为硬阻断，任何越界修改都物理阻断
...
- 获取 Lease（防止多 Agent 同时操作同一任务）
  - **v2.7 新增**：跨平台文件锁（Windows `msvcrt.locking` / Unix `fcntl.flock`）+ 原子写入，彻底杜绝多进程竞争同一 Lease
  - 过期 Lease 自动检测并重新获取
...
- 状态转换后自动生成 Snapshot（用于中断恢复，含沙盒路径）
  - **v2.7 新增**：Snapshot 保留现有上下文（lease、sub_tasks、sandbox、required_context、recovery_prompt），中断恢复时不丢失进度
  - **v2.7 新增**：微秒级时间戳避免并发覆盖
...
| `plan <task-id> --approve [--auto-split]` | Inbox → Plan Ready | G1 + G2 | 批准 plan（G2 超预算时自动语义感知拆分并记录子任务到父 snapshot，检测重复；`--auto-split` 自动确认拆分无需交互） |
| `execute [<task-id>] [--no-sandbox]` | Plan Ready → Executing | G3 | 开始执行，获取 Lease，**自动创建沙盒**（`--no-sandbox` 跳过），**自动标记 plan 第 1 步为 in_progress**（省略 task-id 则自动认领） |
...
| `sandbox create <task-id>` | — | — | 创建沙盒工作区 |
...
> **v2.6 自动沙盒**：使用 `/execute-plan` 时，Agent-Guard 会自动在 `.worktrees/` 下创建 `task/{task-id}` 沙盒，无需手动操作。`finish` 完成后自动销毁并提取 patch。
...
**v2.6 起，TDD 从建议升级为硬宪法规则：**

- 每个 Task 的步骤必须呈现严格的 4 步顺序：**写失败测试 → 运行确认失败 → 写最小实现 → 运行确认通过**
- **禁止先实现后补测试**，plan-schema.yaml 将此视为 error 级违规
- G1 Plan Valid / G2 Complexity Budget 在审查 plan 时，可据此否决不符合 TDD 顺序的计划
- Subagent 必须在完成报告中证明测试先于实现
...
**原因：** G4 v2.7 起检测 staged + unstaged + untracked 三类变更。执行过程中生成的临时文件或日志会被检测到。
...
**原因（v2.7 之前）：** 早期版本在 sandbox 创建失败时未释放 Lease。
